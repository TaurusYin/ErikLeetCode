...
class Solution:
    def nearestValidPoint(self, x: int, y: int, points: List[List[int]]) -> int:
        coor_points = [[points[i][0], points[i][1], i] for i in range(len(points))]
        result = []
        min_distance = sys.maxsize
        for _x, _y, index in sorted(coor_points, key=lambda c: abs(c[0] - x) + abs(c[1] - y)):
            distance = abs(x - _x) + abs(y - _y)
            if (x == _x or y == _y) and distance <= min_distance:
...
if __name__ == '__main__':
    ...
    res = Solution().nearestValidPoint(x, y, points)
    points = [[3, 4]]
    res = Solution().nearestValidPoint(x, y, points)
    print()
